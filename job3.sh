...
#python3 predict_and_eval_all_languages_separately.py
python3.7 metalearn_train.py --meta_lr 5e-5 --inner_lr 1e-4 --updates 1
python3.7 metalearn_train.py --meta_lr 1e-5 --inner_lr 1e-3 --updates 1 --more_lr 1

for f in *; do
    #echo $f
    if [ -d ${f} ]; then
	if [[ $f == metalearn* ]]; then  # [[ $f == finetun* ]]
